const Nature = artifacts.require("./Nature.sol");
const NatureFactory = artifacts.require("./NatureFactory.sol");
...
module.exports = async (deployer, network, addresses) => {
  ...
  if (DEPLOY_CREATURES) {
    await deployer.deploy(Nature, proxyRegistryAddress, {gas: 5000000});
  }

  if (DEPLOY_CREATURES_SALE) {
    await deployer.deploy(NatureFactory, proxyRegistryAddress, Nature.address, {gas: 7000000});
    const creature = await Nature.deployed();
    await creature.transferOwnership(NatureFactory.address);
  }
